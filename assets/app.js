...
(function () {
  ...
  /* Journey strip: you-are-here across the sessions (chairman track of 3, analyst track of 5) */
  var crumb = document.querySelector(".toolbar .crumb");
  var mastheadWrap = document.querySelector(".masthead .wrap");
  if (crumb && mastheadWrap) {
    var mx = crumb.textContent.match(/Chairman session (\d+) of 3/);
    var m = mx || crumb.textContent.match(/Analyst session (\d+) of 5/);
    if (m) {
      var current = parseInt(m[1], 10);
      var pages = mx
        ? ["c1-five-years-in-one-picture.html", "c2-what-went-right-what-went-wrong.html", "c3-decisions-and-the-data-we-owe.html"]
        : ["a1-map-the-data-before-you-read-it.html", "a2-follow-the-margin.html", "a3-products-and-baskets.html", "a4-merchants-and-stores.html", "a5-customers-promotions-and-the-gaps.html"];
...
(function () {
  "use strict";
  var PASSPORT_KEY = "lwp-passport:ship-platform-diagnosis";
  var TOTAL_SESSIONS = 8;
  var SESSION_LABELS = ["c1", "c2", "c3", "a1", "a2", "a3", "a4", "a5"];
  ...
  var FNS = {
    ...
    contrast: function (s) {
      ...
      if (hx.length < 2) return { error: "Give me two hex colours, for example  #FFFFFF #163E6A" };
      function lum(h) {
        h = h.replace("#", "");
  ...
  /* stamps are numbered 1-8 across both tracks: chairman c1-c3 = 1-3, analyst a1-a5 = 4-8 */
  var cmC = crumbEl && crumbEl.textContent.match(/Chairman session (\d+) of 3/);
  var cmA = crumbEl && crumbEl.textContent.match(/Analyst session (\d+) of 5/);
  var here = cmC ? parseInt(cmC[1], 10) : (cmA ? 3 + parseInt(cmA[1], 10) : null);
  ...
  function ppPaintPill() {
    ...
    pill.title = n === 0 ? "Answer all three quiz questions in a session to stamp it."
      : "Stamped: " + Object.keys(got).sort().map(function (k) { return SESSION_LABELS[k - 1]; }).join(", ");
      ...
      var watch = setInterval(function () {
        ...
        if (done >= qs.length) {
          ...
          if (!got[here]) {
            ...
            var score = document.querySelector(".quiz-score");
            if (score) {
              score.textContent += "  🎫 Session " + SESSION_LABELS[here - 1] + " stamped in your passport (" +
  ...
  if (strip) {
    ...
    for (var i = 1; i <= TOTAL_SESSIONS; i++) {
      ...
      a.appendChild(el("b", null, SESSION_LABELS[i - 1]));
    ...
    var note = el("p", "pp-note", ppCount(got2) === 0
      ...
      : ppCount(got2) >= TOTAL_SESSIONS
        ? "Eight of eight. You finished both tracks - the chairman deck and the analyst notebooks."
